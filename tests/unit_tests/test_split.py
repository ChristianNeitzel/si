...
# Evaluation Exercise 6.2: Testing stratified_train_test_split function with the iris dataset.
    def test_stratified_train_test_split(self):
        ...
        original_classes = set(self.dataset.y)  # Retain only the unique labels while excluding duplicates for each set
        train_classes = set(train.y)
        test_classes = set(test.y)

        self.assertEqual(original_classes, train_classes)   # All 3 of these classes should have the same number of uniques
